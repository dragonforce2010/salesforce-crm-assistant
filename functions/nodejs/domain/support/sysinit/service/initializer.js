...
class Initializer {
  ...
  async createAllObjectData() {
    ...
    for(let objectApiName of Object.keys(appInitData)) {
      ...
      appInitData[objectApiName] = appInitData[objectApiName].map(item => {
        for (let key in item) {
          if(key === 'mockDataSelect') {
            const randomIndex = Math.floor(Math.random() * item[key].length)
            item[key] = item[key][randomIndex]
          }
      ...
      const res = await this.client.create(objectApiName, appInitData[objectApiName])
    ...
    // 更新对象记录（主要更新引用关系）
    for(let objectApiName of Object.keys(appInitData)) {
      this.logger.info(`Creating ${objectApiName} related object reference ...`)
      appInitData[objectApiName] = appInitData[objectApiName].map(item => {
        for (let key in metadata[objectApiName].pk) {
          const referenceObjectApiName = metadata[objectApiName].pk[key].objectApiName
          const referenceObjectData = appInitData[referenceObjectApiName]?.filter(item => item.Id)
          if(referenceObjectData.length == 0)
            break
          const randomIndex = Math.floor(Math.random() * referenceObjectData.length)
          item[key] = referenceObjectData[randomIndex].Id
        }
        return item
      })

      const tobeUpdated = appInitData[objectApiName].filter(item => item.Id) || []
      if(tobeUpdated.length > 0) {
        await this.client.update(objectApiName,tobeUpdated)
        this.logger.info(`Updated ${objectApiName} related object reference, total: ${tobeUpdated.length}`)
      }
    }
  }
  ...
  async deleteAllObjectData() {
    for(let objectApiName of Object.keys(metadata)) {
      if(!metadata[objectApiName].allowDelete)
        continue
